...
To use your Grok account instead of an API key, run
`mobilerun configure grok` (add `--device-code --no-browser` on a headless
host). Mobilerun runs its own xAI OAuth flow and never invokes the Grok CLI or
...
**Common CLI flags:**
- `--provider` - LLM provider (GoogleGenAI, OpenAI, XAI, grok_oauth, Anthropic, etc.)
