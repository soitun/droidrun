...
|-----------|------|---------|-------------|
| `config` | `DroidConfig \| None` | `None` | Full config object (loads LLMs from profiles if `llms` not provided) |
...
### Config Format
```python
from droidrun import DroidConfig, CredentialsConfig
