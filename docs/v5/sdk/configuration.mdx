...
```python
from droidrun import AgentConfig, FastAgentConfig, ManagerConfig, ExecutorConfig, AppCardConfig
...
droidrun run "Task" --provider GoogleGenAI --model gemini-3.1-flash-lite-preview
...
- `--agent NAME` - Use an external agent instead of DroidRun's built-in agent. External agents are self-contained modules in `droidrun/agent/external/` that receive raw ADB access and run independently. Configure them under `external_agents` in config.yaml.
