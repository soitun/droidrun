...
| `--connection visual-remote` | Use a compatible visual remote server | Platform default |
...
In screenshot-only mode, Mobilerun does not read an accessibility tree. The agent uses coordinate actions, swipe, supported system buttons, and direct text typing based on screenshots. App launch support depends on the selected connection.
  </Tab>
